...
class GraspExecutor:
    ...
    def _calculate_poses(self, object_data, pose_type):
        ...
        T_B_O = self.robot_controller.get_current_tool_pose() @ np.linalg.inv(self.T_C_6) @ T_C_O
        rospy.loginfo("Calculated T_B_O (Object in Base Frame).")

        T_B_TCP = self._calculate_target_tcp_pose(T_B_O, np.array(object_data['dimensions']), pose_type)
        
        T_B_6 = T_B_TCP @ np.linalg.inv(self.T_TCP_6)
        rospy.loginfo("Calculated T_B_6 (target flange pose) from T_B_TCP.")

        approach_vector = T_B_6[:3, 2] 
        
        T_B_6_approach = T_B_6.copy()
        T_B_6_approach[:3, 3] += approach_vector * APPROACH_DISTANCE

        T_B_6_retreat = T_B_6_approach.copy() 
        rospy.loginfo("Calculated Approach and Retreat poses.")
        
        return {"approach": T_B_6_approach, "grasp": T_B_6, "retreat": T_B_6_retreat}
    ...
    def _execute_grasp_motion(self, poses, pose_type):
        ...
        if not self._move_to_goal(poses["approach"], "APPROACH"): return
        if not self._move_to_goal(poses["grasp"], "GRASP"): return

        self.robot_controller.send_gripper_command(pose_type, position=255)
        rospy.sleep(2.0)

        if not self._move_to_goal(poses["retreat"], "RETREAT"): return
    ...
    def move_to_home_pose(self):
        ...
        self.robot_controller.move_to_joint_goal(HOME_POSE_JOINTS)
